...
void StereoToolsPreset::save(boost::property_tree::ptree& root,
    ...
    root.put(section + ".stereo_tools.side-level",
             settings->get_double("slev"));
...
void StereoToolsPreset::load(boost::property_tree::ptree& root,
    ...
    settings->set_boolean(
        "phasel", root.get<bool>(section + ".stereo_tools.phasel",
                                 get_default<bool>(settings, "phasel")));

    settings->set_boolean(
        "phaser", root.get<bool>(section + ".stereo_tools.phaser",
                                 get_default<bool>(settings, "phaser")));

    settings->set_string(
        "mode",
        root.get<std::string>(section + ".stereo_tools.mode",
                              get_default<std::string>(settings, "mode")));

    settings->set_double(
        "slev", root.get<double>(section + ".stereo_tools.side-level",
                                 get_default<double>(settings, "slev")));

    settings->set_double(
        "sbal", root.get<double>(section + ".stereo_tools.side-balance",
                                 get_default<double>(settings, "sbal")));

    settings->set_double(
        "mlev", root.get<double>(section + ".stereo_tools.middle-level",
    ...
    settings->set_double(
        "sc-level",
        root.get<double>(section + ".stereo_tools.sc-level",
