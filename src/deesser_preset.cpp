...
void DeesserPreset::save(boost::property_tree::ptree& root,
    ...
    root.put(section + ".deesser.sc-listen",
             settings->get_boolean("sc-listen"));
}
...
void DeesserPreset::load(boost::property_tree::ptree& root,
    ...
    settings->set_string(
        "mode",
        root.get<std::string>(section + ".deesser.mode",
    ...
    settings->set_double(
        "f1-level",
        root.get<double>(section + ".deesser.f1-level",
                         get_default<double>(settings, "f1-level")));

    settings->set_double(
        "f2-level",
        root.get<double>(section + ".deesser.f2-level",
                         get_default<double>(settings, "f2-level")));

    settings->set_double(
        "f2-q", root.get<double>(section + ".deesser.f2-q",
