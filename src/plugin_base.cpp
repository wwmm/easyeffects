...
GstPadProbeReturn event_add(GstPad* pad,
...
GstPadProbeReturn event_remove(GstPad* pad,
                               GstPadProbeInfo* info,
                               gpointer user_data) {
    auto l = static_cast<PluginBase*>(user_data);
    ...
    if (enable) {
        gst_pad_add_probe(
            ...
            [](auto pad, auto info, auto d) {
                ...
                if (!plugin) {
                    auto srcpad =
                        gst_element_get_static_pad(l->identity_out, "src");

                    auto probe_type = static_cast<GstPadProbeType>(
                        GST_PAD_PROBE_TYPE_BLOCK |
                        GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM);

                    gst_pad_add_probe(srcpad, probe_type, event_add, l,
                                      nullptr);

                    gst_object_unref(srcpad);

                    /* push EOS into the element, the probe will be fired when
                     * the EOS leaves the effect and it has thus drained all of
                     * its data
                     */

                    auto sinkpad =
                        gst_element_get_static_pad(l->identity_out, "sink");
        ...
        gst_pad_add_probe(
            ...
            [](auto pad, auto info, auto d) {
                ...
                if (plugin) {
                    ...
                    gst_pad_add_probe(srcpad, probe_type, event_remove, l,
                                      nullptr);
